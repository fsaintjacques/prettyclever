/**
 * TD(λ) self-play trainer for the tdnet value network (TD-Gammon style).
 *
 *   npx tsx scripts/train-td.ts --minutes 30
 *   npx tsx scripts/train-td.ts --resume /path/to/td-checkpoint.json --minutes 10
 *
 * Design:
 *  - Afterstate learning: V(s) predicts finalScore/300 for the state left
 *    behind after each decision; chance (dice) is exogenous, and the features
 *    exclude die faces, so roll-phase states are implicit expectations.
 *  - Warm start: a few thousand planner-tuned games regressed on their final
 *    scores (Monte-Carlo targets, streaming) put V in the right ballpark
 *    before self-play begins.
 *  - Self-play: ε-greedy afterstate argmax; after each episode the recorded
 *    afterstates are trained toward forward-view λ-returns
 *    (G_t = (1-λ)·V(s_{t+1}) + λ·G_{t+1}, terminal G = finalScore/300),
 *    which is the offline equivalent of eligibility traces but lets us use
 *    Adam and shuffled minibatches for stability.
 *  - Instrumentation: every --eval-every episodes the current net is frozen
 *    and the pure argmax policy is scored on a fixed eval seed set; the best
 *    net so far is kept and shipped.
 *
 * All randomness flows from seeded mulberry32 streams (resume-reproducible).
 */
...
import { makePlanner, type PlannerOpts } from '../src/strategies/planner';
import {
  chooseByValue,
  createEvalCtx,
  extractFeatures,
...
// Copy of the planner-tuned opts from src/strategies/index.ts (imported here
// literally rather than via the registry so the trainer does not depend on
...
function playEpisode(ctx: EvalCtx, rng: () => number, epsilon: number): Episode {
  ...
  for (;;) {
    ...
    else a = chooseByValue(ctx, s, v, node.actions);
...
function evalNet(net: Net, games: number, seedBase: number): { mean: number; std: number } {
  ...
  for (let i = 0; i < games; i++) {
    ...
    for (;;) {
      ...
      applyActionMut(s, v, chooseByValue(ctx, s, v, node.actions));
    }
...
function writeWeightsModule(path: string, params: TdNetParams, header: string): void {
  ...
  const src = `/**
 * Trained TD(λ) value-network weights — GENERATED by scripts/train-td.ts.
 * ${header}
 */
import type { TdNetParams } from './tdnet';

export const TDNET_WEIGHTS: TdNetParams = {
  nIn: ${params.nIn},
  nH1: ${params.nH1},
  nH2: ${params.nH2},
  W1: ${arr(params.W1)},
...
if (cfg.resume && existsSync(cfg.resume)) {
  ...
  net = initNet(TD_FEATURES, cfg.hidden, cfg.hidden, mulberry32((cfg.seed ^ 0x5eed) >>> 0));
  trainer = new Trainer(net);
}
...
// --- Warm start: regress V on planner-tuned Monte-Carlo returns (streaming).
if (!warmDone) {
  const planner = makePlanner(PLANNER_TUNED, 'planner-tuned');
  const warmRng = mulberry32((cfg.seed ^ 0x9e3779b9) >>> 0);
  const scratch = new Float64Array(net.nIn);
  let scoreSum = 0;
  let err = 0;
  console.log(`warm start: ${cfg.warmGames} planner-tuned games (MC regression)...`);
  for (let g = 0; g < cfg.warmGames; g++) {
    const gameRng = mulberry32((cfg.seed + 101 + g * 2654435761) >>> 0);
    const ep = playPolicyEpisode(
      (s, actions) => planner.choose(s, actions, { variant: v, rng: gameRng }),
      scratch,
      gameRng,
    );
    scoreSum += ep.score;
    ...
    if ((g + 1) % 500 === 0) {
      console.log(
        `  warm ${g + 1}/${cfg.warmGames}  planner mean ${(scoreSum / (g + 1)).toFixed(1)}  tdErr ${err.toFixed(4)}  ${elapsedSec().toFixed(0)}s`,
      );
    }
  }
  warmDone = true;
...
writeWeightsModule(
  cfg.out,
  final,
  `${episode} self-play episodes (λ=${cfg.lambda}, seed ${cfg.seed}), best frozen eval ` +
    `${bestMean.toFixed(1)} on ${cfg.evalGames} games @ seed ${cfg.evalSeed}.`,
);
